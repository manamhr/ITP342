...
//        {
//            self.textLabel.text=@"No flashcards!";
//        }

        return nil;
    }
    else {
        _currentIndex=arc4random() % (self.flashcards.count);
...
- (void) toggleFavorite{
    ...
    Flashcard* fix= [self.flashcards objectAtIndex: self.currentIndex];
    
    if(fix.isFavorite==YES)
    {
        fix.isFavorite = NO;
    }
    ...
    [self.flashcards  replaceObjectAtIndex:self.currentIndex withObject:fix];
...
- (NSArray *) favoriteFlashcards{
    ...
    for (Flashcard* card in self.flashcards)
    {
        if ( [card isFavorite] == YES)
        {
            [favs addObject: card];
        }
    }
